...
SceneParser::SceneParser(const string& yamlPath) {
    ...
    if (simConfigRoot["game"]) {
        ...
        scene.simulationConfig.game.enable_field = gameNode["enable_field"].as<bool>(true);
    ...
    if (scene.simulationConfig.game.enable_field) {
    ...
    // Parse optional objects list
    if (sceneRoot["objects"]) {
        for (const YAML::Node& objNode : sceneRoot["objects"]) {
            ObjectSpec obj;
            obj.name  = objNode["name"].as<string>();
            obj.type  = objNode["type"].as<string>("box");
            obj.mesh  = objNode["mesh"] ? objNode["mesh"].as<string>() : "";
            obj.fixed = objNode["fixed"].as<bool>(false);
            obj.scale = objNode["scale"].as<float>(1.0f);
            if (objNode["position"])
                for (int i = 0; i < 3; ++i) obj.pos[i]  = objNode["position"][i].as<float>();
            if (objNode["size"])
                for (int i = 0; i < 3; ++i) obj.size[i] = objNode["size"][i].as<float>();
            if (objNode["rgba"]) {
                for (int i = 0; i < 4; ++i) obj.rgba[i] = objNode["rgba"][i].as<float>();
                obj.has_rgba = true;
            }
...
string SceneParser::buildMuJoCoXml() {
    ...
    if (!scene.simulationConfig.game.enable_field) {
        // Add checker texture asset
        xml_node asset_no_field = mujoco.child("asset");
        if (!asset_no_field)
            asset_no_field = mujoco.append_child("asset");
        xml_node checkerTex = asset_no_field.append_child("texture");
        checkerTex.append_attribute("name") = "checker_tex";
        checkerTex.append_attribute("type") = "2d";
        checkerTex.append_attribute("builtin") = "checker";
        checkerTex.append_attribute("rgb1") = "0.9 0.9 0.9";
        checkerTex.append_attribute("rgb2") = "0.5 0.5 0.5";
        checkerTex.append_attribute("width") = "512";
        checkerTex.append_attribute("height") = "512";
        xml_node checkerMat = asset_no_field.append_child("material");
        checkerMat.append_attribute("name") = "checker_mat";
        checkerMat.append_attribute("texture") = "checker_tex";
        checkerMat.append_attribute("texrepeat") = "20 20";

        xml_node ground = worldbody.append_child("geom");
        ground.append_attribute("name") = "ground";
        ground.append_attribute("type") = "plane";
        ground.append_attribute("size") = "20 20 0.1";
        ground.append_attribute("material") = "checker_mat";
...
void SceneParser::buildObjects(xml_node& mujoco, xml_node& worldbody) {
    if (scene.objects.empty()) return;

    // Ensure <asset> node exists
    xml_node asset = mujoco.child("asset");
    if (!asset) asset = mujoco.append_child("asset");

    for (const ObjectSpec& obj : scene.objects) {
        // Register mesh asset if needed
        if (obj.type == "mesh") {
            filesystem::path meshPathObj = filesystem::path(PROJECT_ROOT) / "resources" / "meshes" / "objects" / (obj.mesh + ".obj");
            if (!filesystem::exists(meshPathObj))
                throw runtime_error("Object mesh not found: " + meshPathObj.string() +
                                    "\nRun circus/resources/meshes/objects/download_objects.py first.");
            xml_node meshAsset = asset.append_child("mesh");
            meshAsset.append_attribute("name") = obj.mesh.c_str();
            meshAsset.append_attribute("file") = ("objects/" + obj.mesh + ".obj").c_str();
            ostringstream scaleStr;
            scaleStr << obj.scale << " " << obj.scale << " " << obj.scale;
            meshAsset.append_attribute("scale") = scaleStr.str().c_str();
        }

        // Build body (free or fixed)
        xml_node body = worldbody.append_child("body");
        body.append_attribute("name") = obj.name.c_str();
        ostringstream posStr;
        posStr << obj.pos[0] << " " << obj.pos[1] << " " << obj.pos[2];
        body.append_attribute("pos") = posStr.str().c_str();

        if (!obj.fixed)
            body.append_child("freejoint");

        xml_node geom = body.append_child("geom");
        geom.append_attribute("name") = obj.name.c_str();

        // Set rgba only when explicitly specified in YAML, or always for primitives
        if (obj.has_rgba || obj.type != "mesh") {
            ostringstream rgbaStr;
            rgbaStr << obj.rgba[0] << " " << obj.rgba[1] << " " << obj.rgba[2] << " " << obj.rgba[3];
            geom.append_attribute("rgba") = rgbaStr.str().c_str();
        }

        if (obj.type == "mesh") {
            geom.append_attribute("type") = "mesh";
            geom.append_attribute("mesh") = obj.mesh.c_str();
        } else {
            geom.append_attribute("type") = obj.type.c_str();
            ostringstream sizeStr;
            if (obj.type == "sphere") {
                sizeStr << obj.size[0];
            } else {
                sizeStr << obj.size[0] << " " << obj.size[1] << " " << obj.size[2];
            }
            geom.append_attribute("size") = sizeStr.str().c_str();
        }
    }
}
