...
class BridgeNode : public rclcpp::Node {
   public:
    BridgeNode(const std::string& robotName, const std::string& serverIp, int serverPort)
        ...
        // ---- Publisher: Cameras ----
        rgb_camera_pub_ = this->create_publisher<sensor_msgs::msg::Image>("/camera/camera/color/image_raw", 10);
        depth_camera_pub_ = this->create_publisher<sensor_msgs::msg::Image>("/camera/camera/aligned_depth_to_color/image_raw", 10);
        ...
        rgb_image_reader_.configure(shmFilePath_("rgb"));
        depth_image_reader_.configure(shmFilePath_("depth"));
        ...
        init_timer_
            = this->create_wall_timer(3000ms, std::bind(&BridgeNode::publishInitialMessagesWrapper, this));
   ...
   private:
    ...
    rclcpp::Publisher<sensor_msgs::msg::Image>::SharedPtr rgb_camera_pub_;
    rclcpp::Publisher<sensor_msgs::msg::Image>::SharedPtr depth_camera_pub_;
    ...
    std::string shm_dir_;
    ImageSharedMemoryReader rgb_image_reader_;
    ImageSharedMemoryReader depth_image_reader_;
