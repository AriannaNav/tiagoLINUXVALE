...
void BridgeNode::publishSharedImages_() {
    ...
    ImageSharedMemoryReader::Frame rgb_frame{};
    if (rgb_image_reader_.readLatest(rgb_frame) && rgb_frame.channels == 3 && rgb_frame.width > 0 && rgb_frame.height > 0) {
        sensor_msgs::msg::Image rgb_msg{};
        rgb_msg.header.stamp = this->now();
        rgb_msg.header.frame_id = "booster_rgb_cam";
        rgb_msg.width = static_cast<uint32_t>(rgb_frame.width);
        rgb_msg.height = static_cast<uint32_t>(rgb_frame.height);
        rgb_msg.encoding = "rgb8";
        rgb_msg.is_bigendian = false;
        rgb_msg.step = static_cast<sensor_msgs::msg::Image::_step_type>(rgb_frame.width * rgb_frame.channels);
        rgb_msg.data = std::move(rgb_frame.data);
        rgb_camera_pub_->publish(rgb_msg);
    }

    ImageSharedMemoryReader::Frame depth_frame{};
    if (depth_image_reader_.readLatest(depth_frame) && depth_frame.channels == 1 && depth_frame.width > 0 && depth_frame.height > 0) {
        sensor_msgs::msg::Image depth_msg{};
        depth_msg.header.stamp = this->now();
        depth_msg.header.frame_id = "booster_depth_cam";
        depth_msg.width = static_cast<uint32_t>(depth_frame.width);
        depth_msg.height = static_cast<uint32_t>(depth_frame.height);
        depth_msg.encoding = "mono8";
        depth_msg.is_bigendian = false;
        depth_msg.step = static_cast<sensor_msgs::msg::Image::_step_type>(depth_frame.width * depth_frame.channels);
        depth_msg.data = std::move(depth_frame.data);
        depth_camera_pub_->publish(depth_msg);
    }
}
